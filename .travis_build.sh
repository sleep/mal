...
if [ -z "${NO_DOCKER}" ]; then
    ...
    docker run -it -u $(id -u) -v `pwd`:/mal kanaka/mal-test-minimal bash -c "cd miniMAL && node ffi_test2.js" || true
    docker run -it -u $(id -u) -v `pwd`:/mal kanaka/mal-test-minimal node miniMAL/ffi_test2.js || true

    docker run -it -u $(id -u) -v `pwd`:/mal kanaka/mal-test-minimal bash -c "cd miniMAL && node ffi_test3.js" || true
    docker run -it -u $(id -u) -v `pwd`:/mal kanaka/mal-test-minimal node miniMAL/ffi_test3.js || true
else
    make -C ${IMPL}
fi
